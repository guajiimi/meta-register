#!/usr/bin/env python3
"""E2EE fix v4: proper timing — wait for key after Add payment click."""
...
async def main():
    ...
    async with AsyncCamoufox(headless="virtual", humanize=True, os="windows", block_webrtc=True) as browser:
        ...
        # Click Add payment method (this triggers key fetch)
        ...
        print(f"  ✅ Card: {len(result['card'])} chars")
        print(f"  ✅ CVV: {len(result['cvv'])} chars")
        ...
        await page.screenshot(path="data/screenshots/e2ee_fix4.png")
        print("\n[DONE]")

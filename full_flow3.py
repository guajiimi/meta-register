#!/usr/bin/env python3
"""FULL browser flow v3: correct combobox birthday handling."""
...
async def select_combobox(page, label, value):
    """Click combobox and select option by value."""
    # Click the combobox via JS (Camoufox aria-labels not accessible to locator)
    await page.evaluate(f"""
        const cbs = document.querySelectorAll('[role="combobox"]');
        for (const cb of cbs) {{
            if (cb.innerText.toLowerCase().includes('{label}')) {{
                cb.click();
                break;
            }}
        }}
    ...
    await page.evaluate(f"""
        ...
        for (const o of opts) {{
            if (o.innerText.trim() === '{value}') {{ o.click(); break; }}
        }}
    """)
...
async def main():
    ...
    async with AsyncCamoufox(headless="virtual", humanize=True, os="windows", block_webrtc=True) as browser:
            ...
            try:
                ...
                if "get_server_encryption_key" in body and "trust_chain" in body:
                    ...
                    if tc:
                        ...
                        print(f"  [KEY] EC-{cert.public_key().key_size}")
        ...
        # Name
        body = await page.evaluate("document.body?.innerText || ''")
        if "name" in body.lower() and "first" in body.lower():
            print("  → Name")
            inputs = await page.query_selector_all('input[type="text"]')
            if len(inputs) >= 2:
                await inputs[0].fill(first); await inputs[1].fill(last)
            await page.get_by_role("button", name="Next").click()
            await asyncio.sleep(5)

        # Birthday (comboboxes!)
        body = await page.evaluate("document.body?.innerText || ''")
        if "birthday" in body.lower() or "date of birth" in body.lower():
            print("  → Birthday (comboboxes)")
            await select_combobox(page, "year", "1995")
            await select_combobox(page, "month", "March")
            await select_combobox(page, "day", "15")
            await asyncio.sleep(1)
            await page.get_by_role("button", name="Next").click()
            await asyncio.sleep(5)
        ...
        # OTP
        body = await page.evaluate("document.body?.innerText || ''")
        if "code" in body.lower() or "confirm" in body.lower() or "verify" in body.lower():
        ...
        # Handle "Save your login info?" and any post-registration screens
        for _ in range(10):
            body = await page.evaluate("document.body?.innerText || ''")
            url = page.url
            print(f"  [{_}] URL: {url[:80]} | Body: {body[:60]}")
            
            if "dev.meta.ai" in url:
                print(f"  ✅ On dev.meta.ai!")
                break
            
            if "Save" in body and ("login" in body.lower() or "info" in body.lower()):
                for text in ["Save", "OK", "Continue"]:
                    try:
                ...
                await asyncio.sleep(3)
            elif "Welcome" in body:
                await dismiss()
                await asyncio.sleep(2)
            elif "created" in body.lower() or "account" in body.lower():
                # Click Continue/OK/Get started
                for text in ["Continue", "Get started", "OK", "Done"]:
                    try:
                        await page.get_by_role("button", name=text).click(timeout=2000)
                        print(f"  Clicked: {text}")
                        break
                ...
                btns = await page.evaluate("""
                    Array.from(document.querySelectorAll('div[role="button"], button'))
                        .filter(b => b.offsetParent !== null && b.innerText.trim().length > 0 && b.innerText.trim().length < 30)
                        .map(b => b.innerText.trim())
                """)
                if btns and btns[0]:
                    try:
                        await page.get_by_role("button", name=btns[0]).click(timeout=2000)
                        print(f"  Clicked: {btns[0]}")
                    except: pass
                await asyncio.sleep(3)
        
        # If still on auth.meta.com, force navigate to dev.meta.ai
        if "dev.meta.ai" not in page.url:
            print(f"  ⚠ Still on auth.meta.com, navigating to dev.meta.ai...")
